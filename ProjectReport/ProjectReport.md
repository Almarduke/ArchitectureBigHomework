...
2. **关注点的候选模式**
   ...
   - **服务分发的负载均衡**
     ...
     | 3 | 在缓存中存储和维护服务端清单 | 中 | 低 | 中 |

     - **选择的模式及理由**

       使用硬件处理分发请求虽然效率更高，但与快速迭代和部署的现实要求有所冲突；使用Ngnix等软件处理请求分发，可以进行快速的迭代和修改，同时可以很方便地修改配置，同时在正常流量下可以保证与硬件分发相差不大的性能提升。

       但是无论哪种方法，都会在数据量集中访问的情况下出现单点的性能瓶颈问题，可以通过在API网关的缓存中存储和维护服务端清单，来减少API网关对负载均衡节点的访问，提升效率。当注册节点接收到API网关部分的服务请求时，会根据各个服务剩余的资源情况选择合适的服务节点，避免出现单一服务节点流量过大的情况，达到负载均衡。

       最后选择的模式为“使用Ngnix等软件处理请求分发”和“在缓存中存储和维护服务端清单”。
...
![](pics/4.2.5-C&C视图.jpg)

2）Module视图

![](pics/4.2.5-Module视图.jpg)
...
![4.4.5-C&C视图](pics/4.4.5-C&C视图.png)
...
![4.5.5-C&C视图](pics/4.5.5-C&C视图.jpg)

2. **Module视图**

![4.5.5-Module视图](pics/4.5.5-Module视图.jpg)
...
![4.6-CS架构最终模块视图](pics/4.6-CS架构最终模块视图.jpg)
